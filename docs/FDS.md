...
|---|---|---|---|---|
| 2026-06-10 | 0.24 | **Fixed reversed Fibaro smoke/heat/tamper/fault/battery polarity.** The FGSD002 alarm channels are Z-Wave **Switch** (ON=alarm), surfaced on the Number Items as **1=Alarm, 0=OK** — the opposite of the old `0=Alarm` assumption (confirmed by a live smoke+tamper test and the openHAB Z-Wave binding docs). Flipped `alarm.map` (0=OK/1=Alarm); made `smoke.rules` interpret alarm/clear **per source** (Fibaro nonzero=Alarm, Shed unchanged at 0=Alarm); made the Init seeding seed Fibaro→0 / Shed→1 (a global `1` seed would have faked a Fibaro alarm on every boot). Shed MQTT unit unchanged | transform/alarm.map, rules/smoke.rules, items/items.items, docs/FDS.md | Claude Code |
...
| `GK1SA1_smoke/_heat/_tamper/_fault/_battalarm` | Number | MAP(alarm.map) | zwave `GK1-SA1` — UI link (JSONDB) | gSmoke, gSmokeAlarm | Kitchen Fibaro FGSD002 alarm channels: `alarm_smoke`/`_heat`/`_tamper`/`_system`/`_battery` (Switch alarm channel ON=alarm → Number **1=Alarm, 0=OK**) |
| `GK1SA1_temp/_battpc` | Number | °C / % | zwave `GK1-SA1` — UI link (JSONDB) | gSmoke | Kitchen detector `sensor_temperature` / `battery-level` |
| `FL1SA2_smoke/_heat/_tamper/_fault/_battalarm` | Number | MAP(alarm.map) | zwave `FL1-SA2` — UI link (JSONDB) | gSmoke, gSmokeAlarm | Landing Fibaro FGSD002 alarm channels: `alarm_smoke`/`_heat`/`_tamper`/`_system`/`_battery` (Switch alarm channel ON=alarm → Number **1=Alarm, 0=OK**) |
...
gDevices
 ...
 ├─ gSmokeAlarm          (no tag)       → all detector alarm Items, MIXED polarity: 10 Fibaro alarm_* (1=Alarm, 0=OK) + shed GY1SA4_smoke/_tamper (0=Alarm, 1=OK); smoke.rules normalises per source
...
| Smoke Detector Alarm Init | System started | Seed any NULL/UNDEF `gSmokeAlarm` member to its OK value **per source**: Fibaro (GK1SA1/FL1SA2) → `0`, Shed (GY1SA4) → `1` | Z-Wave channels report only on event/wakeup, so unreported alarms show "Unknown" until seeded. Seeding the wrong polarity would fake an alarm on boot |
| Smoke Detector Alarm | `Member of gSmokeAlarm` changed | Normalises polarity per source (Fibaro: nonzero=Alarm/0=OK; Shed: 0=Alarm/1=OK). On Alarm: Pushover **titled `🔥 <Location> Smoke Detector`**, body = the event (`SMOKE DETECTED`/`HEAT DETECTED`/`Tamper alarm`/`System fault`/`Low battery`) — **emergency (pri 2)** for `_smoke`/`_heat` (receipt stored per Item), normal for `_tamper`/`_fault`/`_battalarm`; on `0→1`: `cancelPriorityMessage` the stored emergency receipt + Pushover titled `<Location> Smoke Detector` (no fire), body `✅ <event> cleared` | Smoke/heat/tamper/fault/low-battery alerting for all three detectors (Kitchen/Landing Fibaro + Shed MQTT). Smoke/heat ack-loop auto-cancels on recovery (in-memory receipts; lost on restart → falls back to ack/expire) |
